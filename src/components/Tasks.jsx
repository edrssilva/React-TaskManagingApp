      ...
      {tasks.map((task) => (
        ...
        >
          <button
            ...
            className={`flex items-center gap-2 w-full bg-white rounded-sm shadow-md text-left text-slate-500 font-semibold p-2 pl-4' ${
              task.isCompleted && "line-through"
            }`}
          >
            {task.isCompleted && <CheckIcon color="#64748b" />}
            {task.title}
          </button>
          <Button
            onClick={() => {
